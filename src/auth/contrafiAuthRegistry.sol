...
/**
 * @title Contrafi
 *     @notice This contract manages function-level permissions for relevant Contrafi contracts
 */
contract ContrafiAuthRegistry is AccessControlDefaultAdminRules {
    ...
    constructor(uint48 _defaultAdminTransferDelay, address _defaultAdmin)
     ...
     */
    function setPermission(address _contract, bytes4 _functionSelector, bytes32 _role)
        external
    ...
    function _keyFromAddressAndSelector(address _contract, bytes4 _functionSelector) private pure returns (bytes24) {
